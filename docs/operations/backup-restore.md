...
- весь `/var/lib/guard-daemon-operator`, включая рабочий реестр восстановления и
  постоянный маркер восстановления;
...
**Важно:** сохраняемое состояние может содержать точное двоичное представление
...
называть согласованным после остановки. После создания снимка служба остаётся в
состояниях `disabled` и `masked`; снимок сам по себе не разрешает возвращать
прежний рабочий процесс. Снимайте блокировку `mask` только по соответствующей
процедуре аварийного запуска, обновления или восстановления после проверки
...
реестр оператора управляется отдельно и никогда не попадает под рекурсивный
`chown` восстановленного состояния. Управление секретами и несекретной
конфигурацией выполняется отдельно от резервной копии состояния.
...
Восстановленное состояние в этом выпуске никогда не возвращается в режим
подписи. До помещения прежнего состояния в карантин, извлечения или публикации
восстановленного состояния процедура надёжно и атомарно создаёт принадлежащий
`root` маркер `/var/lib/guard-daemon-operator/live-disabled-after-restore`.
Dry-run и `EMERGENCY_STOP=true` позволяют открыть восстановленное дерево для
наблюдения, но обычный рабочий запуск безопасно завершается с ошибкой до создания
компонента подписи. Процедуры удаления маркера в этом выпуске нет.

До извлечения определите, выполнялась ли после снимка любая рабочая операция хотя
...
для автономного экспертного исследования на изолированном узле без доступа к
сети вне рабочего контура; демон с этим деревом не запускают. В основном
каталоге сохраняют наиболее новое состояние.

Сначала отключите и замаскируйте службу, чтобы запрет автозапуска сохранялся
после перезагрузки, затем остановите службу и только после
подтверждения `MainPID=0` замените оба файла с ключами пустыми файлами
`root:root 0600` через утверждённое средство управления секретами. В публичной
конфигурации установите `DRY_RUN=false` и `EMERGENCY_STOP=true`, сохранив обязательные
`RPC_BROADCAST_HTTP_<N>`. Блокировка `mask` устанавливается до остановки, поэтому
перезагрузка в любой момент до безопасного запуска в аварийном режиме не вернёт
прежнюю рабочую конфигурацию. Следующая процедура выполняется целиком от `root`;
любое несовпадение до извлечения оставляет текущее состояние на месте, а любой
отказ после него оставляет службу остановленной:
...
пытайтесь удалить, заменить или переименовать внешний маркер: восстановленное
дерево остаётся доступным только для наблюдения и экспертного анализа в течение
всего срока поддержки этого выпуска.
