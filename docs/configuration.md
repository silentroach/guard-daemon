...
| `TOKEN_VALUE_RULES_<N>` | Доверенные операторские оценки ценности известных токенов в формате `address:minimum_raw_balance:max_cost_wei`, через запятую. |
...
| Режим | Семантика |
|---|---|
| `known-only` | Безопасный режим по умолчанию: разрешены только известные системе токены с доверенной операторской оценкой ценности. |
| `allowlist` | Watcher разрешает только адреса из обязательного `TOKEN_ALLOWLIST_<N>`; доверенная оценка ценности доступна только пересечению списка с известными токенами. |
...
известные metadata или доверенную оценку ценности. Любая
...
Для каждого известного токена, допускаемого к live-операции, оператор задаёт
...
повторяющееся правило, адрес вне набора известных токенов и превышение network cap
блокируют создание live coordinator до первой подписи.

Эта оценка управляет только допуском к платной операции и не делает отчётность
token contract доверенной. Для любого ERC-20 итог всегда имеет статус
`token-reported`: даже согласованные `balanceOf`, успешный receipt и событие
`Transfer` не доказывают экономическую ценность.
