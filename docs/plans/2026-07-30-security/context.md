...
- Оператор работает в NixOS. Агентам разрешено использовать `nix-shell` и `nix shell` для запуска отсутствующих локально инструментов.
- После завершения Task 01 основной воспроизводимый способ запуска toolchain и проверок — закреплённое окружение `nix develop` из корня репозитория.
- Отсутствие глобально установленного Go, Node.js, `solc`, `gitleaks`, `govulncheck` или другого инструмента не является блокером: инструмент следует запускать из Nix-окружения.
- Одноразовый незакреплённый `nix shell nixpkgs#<package>` допустим только для исследования до фиксации версии. Доказательства приёмки и CI должны использовать версии из `flake.lock` и lock-файлов репозитория.
