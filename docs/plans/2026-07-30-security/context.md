...
- Оператор работает в NixOS. Агентам разрешено локально использовать
  `nix develop`, `nix-shell` и `nix shell`, если нужный инструмент не установлен
  глобально. Это удобство разработки, а не обязательная часть проекта.
- GitHub Actions и иные CI environments не устанавливают и не вызывают Nix.
  CI использует официальные setup actions, закреплённые полными commit SHA,
  точные версии Go/Node.js/npm и версии security tools, указанные в workflow.
- Repository commands (`make go-ci`, `make node-ci`, `make check` и отдельные
  targets) не зависят от Nix и должны работать в обычном shell после установки
  документированных tool versions.
- Воспроизводимость CI определяется `go.mod`/`go.sum`, `package-lock.json`,
  точными версиями compiler/security tools и полными SHA GitHub Actions.
  `flake.nix`/`flake.lock` остаются только дополнительным локальным окружением.
- Одноразовый незакреплённый `nix shell nixpkgs#<package>` допустим локально
  только для исследования. Acceptance evidence должно включать независимый от
  Nix CI-compatible запуск repository commands.
