...
`scripts/verify-systemd-account.sh` из проверенного набора инструментов и
отклонить учётную запись, если она допускает вход, не заблокирована, не является
системной или включена в дополнительные группы.
...
- доступный только `root` каталог `/etc/guard-daemon/credentials` и пустые файлы
  с ключами
  `source-private-key` и `sponsor-private-key` с владельцем `root:root` и
  режимом `0600`;
- постоянный каталог состояния `/var/lib/guard-daemon` с режимом `0700`;
- операторский каталог `/var/lib/guard-daemon-operator` с режимом `0755` и
  владельцем `root:root`; tmpfiles не создаёт постоянный маркер восстановления;
- канонический рабочий реестр восстановления
  `/var/lib/guard-daemon-operator/deployment-recovery` с режимом `0700` и
  владельцем `root:root`;
- канонический промежуточный каталог рабочих манифестов
...
`GUARD_DAEMON_CONFIG=/etc/guard-daemon/guard-daemon.env`, а двоичный файл читает
публичную конфигурацию после запрета дампа памяти. Приватные ключи поступают через
`LoadCredential` из доступного только `root` каталога
`/etc/guard-daemon/credentials/`; начальное окружение процесса содержит только
пути и не содержит значений ключей. Файл
состояния пакета `/usr/lib/guard-daemon/guard-daemon.state.env` содержит только
...
CLI развёртывания для сетей с идентификаторами `1`, `56` и `137` запускает назначенный
оператор с правами `root` из проверенного каталога
`/usr/lib/guard-daemon/deployment/<commit>` и всегда использует фиксированные
каталоги восстановления и манифестов. Полный кандидат сохраняется в принадлежащем
`root` каталоге `/usr/lib/guard-daemon/candidates/<commit>`. Пользователь службы
`guard-daemon` не может просматривать эти операторские каталоги, подтверждать,
удалять или архивировать записи. После независимой сверки хеша, nonce и квитанции оператор
вручную архивирует подтверждённую запись вне реестра; автоматизированному агенту
это действие запрещено.

Процедура восстановления до изменения состояния атомарно создаёт
...
может выполнить `lstat` для маркера, но не может создать, заменить или удалить
его.
...
выполняются от `root`. Процесс работает как `guard-daemon` без механизма Linux
capabilities и без новых привилегий. Публичный файл конфигурации обязателен и
читается процессом, но изменяется только `root`; рекомендуемый режим `0640`, владелец `root`,
группа `guard-daemon`. Файлы с ключами остаются `root:root 0600`; systemd
копирует их через `LoadCredential`, а двоичный файл отклоняет
`SOURCE_PRIVATE_KEY` и `SPONSOR_PRIVATE_KEY` в начальном окружении.
...
`/var/lib/guard-daemon-operator` доступным только для чтения, делает вложенные
каталоги восстановления и манифестов недоступными процессу и не включает дерево
операторских каталогов в `ReadWritePaths`.
`PrivateTmp=no` задан намеренно: приватный `/var/tmp` разрушил бы общую для
хоста блокировку и позволил
бы двум процессам считать себя единственным владельцем одной роли `sponsor`.

`LimitCORE=0` является внешним ограничением unit. До чтения публичной конфигурации и
файлов с ключами двоичный файл Linux дополнительно устанавливает собственные
значения жёсткого и мягкого пределов `RLIMIT_CORE=0`, а также
`PR_SET_DUMPABLE=0`; невозможность применить любую из этих
политик блокирует запуск. Поэтому сборщик данных об аварийных завершениях,
подключённый через pipe, не получает дамп памяти, содержимое файлов с ключами или окружение
рабочего процесса, даже когда глобальный `kernel.core_pattern` направлен в такой
сборщик.
...
Базовый unit не содержит `IPAddressDeny=`. RPC оператора заранее неизвестны,
...
`DRY_RUN`, добавлять ключи, пополнять `sponsor`, создавать рабочий
